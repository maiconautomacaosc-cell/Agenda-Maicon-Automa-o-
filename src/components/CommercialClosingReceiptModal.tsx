...
import { appointmentCommercialValue, closingSubtotal, closingTotal } from '../utils/commercialClosings';
...
export const CommercialClosingReceiptModal: React.FC<Props> = ({ closing, client, appointments, payment, onClose }) => {
  ...
  const serviceItems = closingAppointments.map(a => ({
    id: a.id,
    description: a.serviceTypeName || a.description || a.lockModel || 'Serviço',
    detail: [a.serviceOrder, a.lockModel].filter(Boolean).join(' • ') || a.serialNumber || '',
    amount: appointmentCommercialValue(closing, a) ?? 0,
  }));
  const extraItems = (closing.extraItems || []).map(x => ({
    id: x.id,
    description: x.description || 'Acréscimo',
    detail: 'Acréscimo / valor extra',
    amount: Number(x.amount || 0),
  }));
  const extrasTotal = extraItems.reduce((sum, x) => sum + x.amount, 0);
  const subtotal = closingSubtotal(closing, appointments);
  const discountAmount = Math.max(0, closing.discountType === 'percentual'
    ? subtotal * ((Number(closing.discountValue) || 0) / 100)
    : (Number(closing.discountValue) || 0));
  const discountLabel = closing.discountType === 'percentual' && Number(closing.discountValue) > 0
    ? `Desconto (${Number(closing.discountValue).toLocaleString('pt-BR')}%)`
    : 'Desconto';
  const compositionRows = [...serviceItems, ...extraItems];
  const compositionRowsHtml = compositionRows.map(item => `<div class="line"><div><b>${escapeHtml(item.description)}</b>${item.detail ? `<small>${escapeHtml(item.detail)}</small>` : ''}</div><strong>${escapeHtml(formatCurrencyBRL(item.amount))}</strong></div>`).join('');
  ...
  const saveImage = async () => {
    ...
    const currentWidth = Math.max(1, node.getBoundingClientRect().width);
    const captureHeight = Math.max(1050, Math.ceil(node.scrollHeight * (targetWidth / currentWidth)));
    clone.style.width = `${targetWidth}px`; clone.style.maxWidth=`${targetWidth}px`; clone.style.margin='0';
    const wrap = document.createElement('div'); wrap.setAttribute('xmlns','http://www.w3.org/1999/xhtml'); wrap.style.width=`${targetWidth}px`; wrap.style.background='#fff'; wrap.appendChild(clone);
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${targetWidth}" height="${captureHeight}"><foreignObject width="100%" height="100%">${new XMLSerializer().serializeToString(wrap)}</foreignObject></svg>`;
    const blob = new Blob([svg], {type:'image/svg+xml;charset=utf-8'}); const url=URL.createObjectURL(blob); const img=new Image();
    img.onload=()=>{ const canvas=document.createElement('canvas'); canvas.width=targetWidth*2; canvas.height=captureHeight*2; const ctx=canvas.getContext('2d'); if(!ctx){URL.revokeObjectURL(url);return;} ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.drawImage(img,0,0,canvas.width,canvas.height);URL.revokeObjectURL(url);const link=document.createElement('a');link.download=`recibo-${receiptCode}-${sanitize(client.name)}.png`;link.href=canvas.toDataURL('image/png',1);link.click(); };
    img.src=url;
  };
  ...
  const printReceipt = () => {
    ...
    *{box-sizing:border-box}html,body{margin:0;width:210mm;min-height:297mm;font-family:Arial,sans-serif;background:white;color:#111827}.sheet{width:210mm;min-height:297mm;padding:0 14mm 11mm}.top{height:42mm;margin:0 -14mm 9mm;padding:8mm 14mm;display:flex;align-items:center;justify-content:space-between;background:linear-gradient(120deg,#05090d,#071722,#0b3040);border-bottom:3px solid #06b6d4}.logo{width:54mm;height:27mm;object-fit:contain;object-position:left center}.brand{text-align:right;color:white}.brand h1{margin:0;font-size:18pt}.brand p{margin:2mm 0 0;font-size:8pt;color:#a5f3fc}.title{display:flex;justify-content:space-between;align-items:center;margin-bottom:7mm}.title h2{font-size:20pt;margin:0}.title small{color:#64748b}.tag{padding:2.5mm 4mm;border-radius:20px;background:#ecfeff;color:#0e7490;font-weight:700;font-size:8pt}.grid{display:grid;grid-template-columns:1fr 1fr;gap:3mm}.card{padding:3.5mm;border:1px solid #e2e8f0;border-radius:4mm}.wide{grid-column:1/-1}.label{font-size:7pt;text-transform:uppercase;color:#64748b;font-weight:700;letter-spacing:.4px}.value{font-size:10pt;font-weight:700;margin-top:1.3mm}.money{margin-top:6mm;padding:6mm;border-radius:5mm;background:#0f172a;color:white}.big{font-size:22pt;font-weight:900;color:#5eead4;margin:2mm 0 5mm}.money-grid{display:grid;grid-template-columns:1fr 1fr;gap:3mm}.money-box{border:1px solid #334155;border-radius:3mm;padding:3mm}.k{font-size:7pt;color:#94a3b8}.v{font-size:12pt;font-weight:800;margin-top:1mm}.breakdown{margin-top:5mm;border:1px solid #e2e8f0;border-radius:4mm;overflow:hidden}.breakdown-title{padding:3mm 4mm;background:#f8fafc;font-size:8pt;text-transform:uppercase;color:#475569;font-weight:800;letter-spacing:.4px}.line{display:flex;justify-content:space-between;gap:6mm;align-items:flex-start;padding:2.4mm 4mm;border-top:1px solid #e2e8f0;font-size:8.5pt}.line:first-of-type{border-top:0}.line>div{min-width:0}.line b{display:block}.line small{display:block;color:#64748b;margin-top:.6mm;font-size:7pt}.line strong{white-space:nowrap}.sumline{display:flex;justify-content:space-between;gap:5mm;padding:2mm 4mm;font-size:8pt}.sumline.extra{color:#0f766e}.sumline.discount{color:#b45309}.sumline.total{border-top:1px solid #cbd5e1;margin-top:1mm;padding-top:2.8mm;font-size:10pt;font-weight:900}.statement{margin-top:5mm;padding:3.5mm;border-left:3px solid #06b6d4;background:#f8fafc;font-size:8.5pt;line-height:1.4}.footer{margin-top:5mm;border-top:1px solid #e2e8f0;padding-top:3mm;display:flex;justify-content:space-between;color:#64748b;font-size:7pt}@page{size:A4 portrait;margin:0}@media print{html,body{width:210mm;height:297mm;overflow:hidden}.sheet{height:297mm;min-height:0;padding:0 11mm 7mm;transform:none}.top{height:40mm;margin:0 -11mm 5mm;padding:6mm 11mm;background:linear-gradient(120deg,#05090d 0%,#071722 55%,#0b3040 100%);border-bottom:3px solid #06b6d4;-webkit-print-color-adjust:exact;print-color-adjust:exact}.logo{width:50mm;height:26mm;object-fit:contain;object-position:left center}.brand{color:#fff!important}.brand h1{font-size:17pt;color:#fff!important;font-weight:800;letter-spacing:.3px}.brand p{font-size:7.5pt;color:#a5f3fc!important}.title{margin-bottom:4mm}.title h2{font-size:17pt}.grid{gap:2mm}.card{padding:2.3mm 3mm;border-radius:3mm}.value{font-size:9pt;margin-top:.8mm}.breakdown{margin-top:3mm}.breakdown-title{padding:2mm 3mm}.line{padding:1.5mm 3mm}.sumline{padding:1.3mm 3mm}.sumline.total{padding-top:1.8mm}.money{margin-top:3mm;padding:3.5mm}.big{font-size:18pt;margin:1mm 0 2.5mm}.money-grid{gap:2mm;margin-top:0}.money-box{padding:2mm}.statement{margin-top:3mm;padding:2.5mm}.footer{margin-top:3mm;padding-top:2mm}.actions{display:none!important}}
    </style></head><body><div class="sheet" style="position:relative"><div class="top"><img class="logo" src="${logoMaicon}"/><div class="brand"><h1>MAICON AUTOMAÇÃO</h1><p>Instalação de Fechaduras Eletrônicas</p></div></div><div class="title"><div><h2>RECIBO DE PAGAMENTO</h2><small>Comprovante nº ${escapeHtml(receiptCode)}</small></div><div class="tag">${isPaid?'SERVIÇO QUITADO':'PAGAMENTO PARCIAL'}</div></div><div class="grid"><div class="card"><div class="label">Cliente</div><div class="value">${escapeHtml(client.name)}</div></div><div class="card"><div class="label">Data do recebimento</div><div class="value">${escapeHtml(formatDateBR(payment.date))}</div></div><div class="card"><div class="label">Fechamento comercial</div><div class="value">${escapeHtml(closing.id)}</div></div><div class="card"><div class="label">Ordens de serviço</div><div class="value">${escapeHtml(osList)}</div></div><div class="card wide"><div class="label">Equipamentos / MA</div><div class="value">${escapeHtml(maList)}</div></div><div class="card wide"><div class="label">Referente a</div><div class="value">${escapeHtml(reference)}</div></div><div class="card"><div class="label">Tipo do recebimento</div><div class="value">${escapeHtml(paymentKindLabel(payment.kind))}</div></div><div class="card"><div class="label">Forma de pagamento</div><div class="value">${escapeHtml(paymentMethodLabel(payment.method))}</div></div><div class="card wide"><div class="label">Observação</div><div class="value">${escapeHtml(payment.note?.trim() || '—')}</div></div></div><div class="breakdown"><div class="breakdown-title">Produtos / serviços e composição do fechamento</div>${compositionRowsHtml}<div class="sumline extra"><span>Acréscimos / valores extras</span><b>${escapeHtml(formatCurrencyBRL(extrasTotal))}</b></div><div class="sumline discount"><span>${escapeHtml(discountLabel)}</span><b>${discountAmount > 0 ? '- ' : ''}${escapeHtml(formatCurrencyBRL(discountAmount))}</b></div><div class="sumline total"><span>Total do fechamento</span><span>${escapeHtml(formatCurrencyBRL(total))}</span></div></div><div class="money"><div class="label">Valor recebido neste comprovante</div><div class="big">${escapeHtml(formatCurrencyBRL(payment.amount))}</div><div class="money-grid"><div class="money-box"><div class="k">Valor total do fechamento</div><div class="v">${escapeHtml(formatCurrencyBRL(total))}</div></div><div class="money-box"><div class="k">Saldo após este pagamento</div><div class="v">${escapeHtml(formatCurrencyBRL(balanceAfter))}</div></div></div></div><div class="statement">Recebemos de <b>${escapeHtml(client.name)}</b> o valor de <b>${escapeHtml(formatCurrencyBRL(payment.amount))}</b>, referente ao fechamento <b>${escapeHtml(closing.id)}</b>, pago por <b>${escapeHtml(paymentMethodLabel(payment.method))}</b>.</div><div class="footer"><span>Documento gerado eletronicamente pelo sistema Maicon Automação.</span><span>${escapeHtml(closing.id)} • ${escapeHtml(receiptCode)}</span></div></div><script>window.onload=()=>setTimeout(()=>window.print(),350)</script></body></html>`;
    ...
    <div className="w-full max-w-2xl my-2 sm:my-4">
      ...
      <div id="closing-receipt-card" className="bg-white text-slate-900 rounded-2xl overflow-hidden shadow-xl border border-slate-200 relative">
        ...
        <div className="rounded-2xl border border-slate-200 overflow-hidden">
          <div className="px-3 py-2 bg-slate-50 text-[10px] uppercase tracking-wider font-black text-slate-600">Produtos / serviços e composição do fechamento</div>
          <div className="divide-y divide-slate-200">
            {compositionRows.map(item => <div key={item.id} className="px-3 py-2 flex items-start justify-between gap-3 text-xs"><div className="min-w-0"><div className="font-bold text-slate-800 break-words">{item.description}</div>{item.detail && <div className="text-[10px] text-slate-500 mt-0.5">{item.detail}</div>}</div><div className="font-black text-slate-800 whitespace-nowrap">{formatCurrencyBRL(item.amount)}</div></div>)}
          </div>
          <div className="border-t border-slate-200 bg-slate-50 px-3 py-2 space-y-1.5 text-xs">
            <div className="flex justify-between gap-3 text-teal-700"><span>Acréscimos / valores extras</span><b>{formatCurrencyBRL(extrasTotal)}</b></div>
            <div className="flex justify-between gap-3 text-amber-700"><span>{discountLabel}</span><b>{discountAmount > 0 ? '- ' : ''}{formatCurrencyBRL(discountAmount)}</b></div>
            <div className="flex justify-between gap-3 pt-1.5 border-t border-slate-300 text-sm font-black text-slate-900"><span>Total do fechamento</span><span>{formatCurrencyBRL(total)}</span></div>
          </div>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900 text-white"><div className="text-[10px] uppercase text-slate-400 font-bold">Valor recebido neste comprovante</div><div className="text-2xl sm:text-3xl font-black text-emerald-300 mt-1">{formatCurrencyBRL(payment.amount)}</div><div className="grid grid-cols-2 gap-2 mt-3"><div className="p-2 rounded-xl border border-slate-700"><div className="text-[10px] text-slate-400">Total do fechamento</div><b>{formatCurrencyBRL(total)}</b></div><div className="p-2 rounded-xl border border-slate-700"><div className="text-[10px] text-slate-400">Saldo após pagamento</div><b>{formatCurrencyBRL(balanceAfter)}</b></div></div></div>
