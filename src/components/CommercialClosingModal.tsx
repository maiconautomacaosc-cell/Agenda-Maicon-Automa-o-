...
export const CommercialClosingModal:React.FC<Props>=({closing,client,appointments,allClosings=[],onSave,onClose})=>{
 ...
 const initialReceived=closing.payments.reduce((s,p)=>s+Number(p.amount||0),0);
 const initialTotal=closingTotal(closing,appointments);
 ...
 const toggleOS=(id:string)=>{
   if(paidProtected) return;
   const otherOpen=openClosingByAppointment.get(id);
   if(otherOpen && !draft.appointmentIds.includes(id) && !confirm(`Esta OS já pertence ao ${otherOpen}, que ainda está aberto. Deseja movê-la para ${draft.id}?`)) return;
   setDraft(d=>{
 ...
 const setAppointmentValue=(id:string, raw:string)=>{ if(paidProtected) return; setDraft(d=>({
   ...d,
 ...
 return <div className="fixed inset-0 z-[80] bg-black/90 backdrop-blur-sm overflow-y-auto p-3"><div className="max-w-2xl mx-auto my-4 bg-zinc-950 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl">
  ...
  <div className="p-4 space-y-4">
   {paidProtected && <div className="p-3 rounded-2xl border border-emerald-800/60 bg-emerald-950/20 text-emerald-200 text-xs"><b>Fechamento pago e protegido.</b> Este histórico está bloqueado para evitar alterar OS, valores ou pagamentos por engano. Recibos continuam disponíveis.<button onClick={()=>{if(confirm('Deseja liberar a correção deste fechamento pago? Alterações podem modificar o histórico financeiro e o saldo do cliente.')) setCorrectionUnlocked(true);}} className="mt-2 w-full py-2 rounded-xl border border-amber-800/60 bg-amber-950/30 text-amber-300 font-bold">Liberar correção de valores/pagamentos</button></div>}
   <div className="p-3 rounded-2xl bg-zinc-900 border border-zinc-800">
    ...
    {clientOS.length===0?<div className="text-sm text-zinc-500">Nenhuma OS encontrada.</div>:clientOS.map(a=>{
      ...
      return <div key={a.id} className={`w-full mb-2 p-3 rounded-xl border ${selected?'border-cyan-500 bg-cyan-950/30':'border-zinc-800 bg-zinc-950'}`}>
        <button onClick={()=>toggleOS(a.id)} disabled={paidProtected} className={`w-full text-left ${paidProtected?'cursor-default':''}`}>
          <div className="flex justify-between gap-2"><div><div className="text-[10px] uppercase tracking-wide text-zinc-500">Ordem de serviço</div><b className="text-white">{a.serviceOrder}</b><div className="text-xs text-zinc-400">{formatDateBR(a.date)} • {a.serviceTypeName}</div><div className="text-[11px] text-zinc-500">{(a.equipment||[]).map(e=>e.serialNumber).filter(Boolean).join(' • ') || a.serialNumber || 'sem MA'}</div></div><div className="text-right"><div className={`text-xs font-bold ${selected?'text-cyan-300':'text-zinc-500'}`}>{selected?'Vinculada':(openClosingByAppointment.get(a.id)?`Em ${openClosingByAppointment.get(a.id)} aberto • toque para unir`:'Toque para vincular')}</div></div></div>
        </button>
        {selected && <div className="mt-3 pt-3 border-t border-cyan-900/50">
          ...
          <div className="flex items-center gap-2"><span className="text-zinc-500 font-bold">R$</span><input type="number" step="0.01" min="0" value={value==null?'':String(value)} onChange={e=>setAppointmentValue(a.id,e.target.value)} disabled={paidProtected} placeholder="Ainda não definido" className="flex-1 disabled:opacity-60 bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2 text-white"/></div>
   ...
   <div className="p-3 rounded-2xl bg-zinc-900 border border-zinc-800">
    ...
    {draft.payments.length===0?<div className="text-sm text-zinc-500 mb-3">Nenhum recebimento lançado.</div>:draft.payments.map(p=><div key={p.id} className={`w-full flex justify-between items-start gap-3 py-3 border-b border-zinc-800 ${editingPaymentId===p.id?'bg-amber-950/20':''}`}>
      <button onClick={()=>!paidProtected&&editPayment(p)} disabled={paidProtected} className="flex-1 text-left min-w-0 disabled:cursor-default"><div className="text-zinc-200 font-bold">{paymentKindLabel(p.kind)}</div><div className="text-xs text-zinc-500">{formatDateBR(p.date)} • {paymentMethodLabel(p.method)}</div>{p.origin==='migrado_os' && <div className="text-[11px] text-amber-300 mt-0.5">Origem: migrado da OS {p.sourceServiceOrder||''}</div>}{p.note&&<div className="text-[11px] text-zinc-500">{p.note}</div>}{!paidProtected&&<div className="text-[10px] text-cyan-400 mt-1">Toque para editar ou excluir</div>}</button>
    ...
    {!paidProtected && <div className="mt-3 p-3 rounded-xl bg-zinc-950 border border-zinc-800 space-y-2">
   ...
   <div className="flex gap-2"><button onClick={onClose} className="flex-1 py-3 rounded-xl bg-zinc-900 text-zinc-300">{paidProtected?'Fechar':'Cancelar'}</button>{!paidProtected&&<button onClick={saveClosing} className="flex-[2] py-3 rounded-xl bg-cyan-500 text-black font-black">Salvar fechamento</button>}</div>
  </div></div>
